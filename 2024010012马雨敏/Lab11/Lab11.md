...
**重要提醒**：
- 只提交 `Lab11.md` 和 PNG 格式截图文件
- 不要提交实验过程中生成的密钥、消息、签名、密文、解密结果等文件
- 确保所有截图清晰可读
- 确保所有思考题都有回答
...
✅ **实践层面**：
- 掌握使用 OpenSSL 生成签名密钥对和加密密钥对
- 掌握对密文进行数字签名和验证签名
- 验证了签名的完整性保护和不可伪造性
- 掌握使用 RSA 公钥加密文件、验证签名后使用 RSA 私钥解密文件

✅ **应用层面**：
- 了解数字签名在软件发布、代码签名、电子邮件等场景中的应用
- 理解"加密 -> 哈希 -> 签名 -> 验证 -> 解密"流程中每一步的作用
- 理解数字签名与数字证书的关系（下一个实验 Lab12 将深入学习）

数字签名是现代密码学的基石之一。在下一个实验（Lab12：数字证书）中，我们将学习如何使用数字签名构建信任链，实现更复杂的身份认证系统。
